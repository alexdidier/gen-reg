classdef GenReg_constr_poly
    %GENREG_constr_poly Generalised dynamic regret controller with
    %constraints and polytopic disturbance bounds
    %   Class for solving the generalised dynamic regret problem for
    %   bounded polytopic disturbances with state and input constraints
    ...
    methods
        function obj = GenReg_constr_poly(sys, params, weight, solver)
            %GENREG_CONSTR_POLY Construct an instance of this class
            %   Construct GenReg_constr_poly Class
            %%% Parse inputs %%%
            switch nargin
                case 2
                    weight='dr';
            ...
            if ischar(weight)
                switch weight
                    case 'dr'
                        obj.name='DR_constr_poly';
                        W_w=eye(n*(T+1));
                    case 'cr'
                        obj.name='CR_constr_poly';
                        W_w=O;
                    otherwise
                        error('Not a valid generalised regret case.')
                end
            else
                obj.name='GR_constr_poly';
                W_w=weight;
            end
