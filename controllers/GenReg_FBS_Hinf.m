classdef GenReg_FBS_Hinf
    %GENREG_FBS Generalised dynamic regret controller using the FBS-Proc
    %   Class for solving the generalised dynamic regret problem for
    %   pointwise in time bounded ellipsdal disturbances using the full
    %   block S-Procedure
    ...
    methods
        function obj = GenReg_FBS_Hinf(sys, params, weight, solver)
            %GENREG_FBS Construct an instance of this class
            ...
            if ischar(weight)
                switch weight
                    case 'dr'
                        obj.name='DR_FBS';
                        W_w=eye(n*(T+1));
                        gamma = params.gammadrFBS;
                    case 'cr'
                        obj.name='CR_FBS';
                        W_w=O;
                        gamma = params.gammacrFBS;
                    otherwise
            ...
            for i=1:T
                Sigma{i} = sdpvar(n*T);
                Sigma_c{i}=sdpvar(n*T,1);
            end
            for i=1:T
                Sigma2{i} = sdpvar(n*T);
                Sigma_c2{i}=sdpvar(n*T,1);
            end
            ...
            for i=1:T
                ...
                lambdaP = lambdaP + temp^0.5*Sigma{i}*temp^0.5;
            end
            lambdaP2 = zeros(n*T);
            for i=1:T
                temp=zeros(n*T);
                temp((i-1)*n+1:i*n,(i-1)*n+1:i*n)=obj.params.P;
                lambdaP2 = lambdaP2 + temp^0.5*Sigma2{i}*temp^0.5;
            end
            constraints = [constraints, lambda(:)>=0,lambda2(:)>=0];
            for i=1:T
                %constraints = [constraints, [lambda(i),Sigma_c{i}';Sigma_c{i},Sigma{i}]>=0];
                %constraints = [constraints, [lambda(i),Sigma_c{i}';Sigma_c{i},Sigma{i}]<=lambda(i)*eye(size([lambda(i),Sigma_c{i}';Sigma_c{i},Sigma{i}]))];
                constraints = [constraints, Sigma{i}>=0, Sigma{i} <= lambda(i)*eye(size(Sigma{i})), Sigma2{i}>=0, Sigma2{i} <= lambda2(i)*eye(size(Sigma2{i}))];
            end
