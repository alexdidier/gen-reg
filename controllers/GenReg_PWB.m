classdef GenReg_PWB
    ...
    methods
        function obj = GenReg_PWB(sys, params, weight, solver)
...
%             %objective = gamma;
% 
%             %objective = -trace((value(Phi_w)'*C*value(Phi_w)-O_3)*W)-2*(x_0'*value(Phi_0)'*C*value(Phi_w)-x_0'*O_2')*w-x_0'*(value(Phi_0)'*C*value(Phi_0)-O_1)*x_0;
%             objective = -w'*(value(Phi_w)'*C*value(Phi_w)-O_3)*w-2*(x_0'*value(Phi_0)'*C*value(Phi_w)-x_0'*O_2')*w-x_0'*(value(Phi_0)'*C*value(Phi_0)-O_1)*x_0;
%             %constraints =[[W,w;w',1]>=0];
%              constraints =[];
%             for i=1:T
%                 temp=zeros(n*T);
%                 temp((i-1)*n+1:i*n,(i-1)*n+1:i*n)=obj.params.P;
%                 A_{i}=temp;
%                constraints = [constraints, w'*temp*w <= 1]; 
%             end
%            % [a]=optimize(constraints, objective, sdpsettings('solver','bmibnb','bmibnb.maxiter',500,'verbose',1));
%            % a
%            % -value(objective)/([x_0;value(w)]'*W_w*[x_0;value(w)])
%            % value(w)
%             %rank(value([W,w;w',1]))
%             A_0=-(value(Phi_w)'*C*value(Phi_w)-O_3);
%             b_0=(x_0'*value(Phi_0)'*C*value(Phi_w)-x_0'*O_2')';
%             A_star = A_0;
%             for i=1:T
%                 A_star = A_star+value(lambda(i))*A_{i};
%             end
%             eig(A_star)
%             value(lambda)
%             obj.A_star = A_star;
