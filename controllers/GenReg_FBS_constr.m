classdef GenReg_FBS_constr
    %GENREG_FBS_CONSTR Generalised dynamic regret controller using the
    %FBS-Proc. for constrained problems
    ...
    %   block S-Procedure with state and input constraints
    ...
    methods
        function obj = GenReg_FBS_constr(sys, params, weight, solver)
            %GENREG_FBS_CONSTR Construct an instance of this class
            ...
            if ischar(weight)
                switch weight
                    case 'dr'
                        obj.name='DR_FBS_constr';
                        W_w=eye(n*(T+1));
                    case 'cr'
                        obj.name='CR_FBS_constr';
                        W_w=O;
                    otherwise
                        error('Not a valid generalised regret case.')
                end
            else
                obj.name='GR_FBS_constr';
                W_w=weight;
            end
            ...
            for i=1:T
                constraints = [constraints, Sigma{i}>=0.5*lambda(i)*eye(size(Sigma{i})), Sigma{i} <= lambda(i)*eye(size(Sigma{i}))];
            end
